...
### 关键认知
1. **我原以为要在代码里写一堆 if-else 判断用户意图，结果一行都不用写**。
   模型自己决定该不该用工具、用哪个、传什么参数，
   我的代码只负责"执行工具 + 循环"，这才叫 Agent。

2. **控制权发生了转移**：代码里没有任何一行 `if 用户问天气 then 调天气工具`。
   该不该调、调哪个、传什么参数，全部由模型决定——代码退化为"执行器 + 循环"。
3. **工具给事实，模型给推理**：工具只返回 `晴，22°C，北风 2 级` 一行字，
   而"22 度穿短袖是否合适"的分层建议完全是模型自己推理生成的。
4. **模型只输出"调用意图"**，真正执行工具的是我们的代码 —— 这条边界就是
   Agent 的安全边界（可在执行前加参数校验与权限控制）。
5. **`description` 就是新的代码**：模型靠工具描述来决定用哪个工具，描述写得含糊就会选错。
